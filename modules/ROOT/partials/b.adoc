...
You will also need to type an empty attribute in front, such as `+{blank}+`, to escape them to 'escape' them from the normal Asciidoc processing (you could use `+` at each end, but this will escape everything inside the brackets, so URLs and such won't work either).
